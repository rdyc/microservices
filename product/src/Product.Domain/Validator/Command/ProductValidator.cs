...
namespace Product.Domain.Validator.Command
{
        ...
        where T : ProductCommand
    {
        ...
        protected void ValidateId()
        {
            RuleFor(c => c.Id)
                .NotNull()
                .MustBeExistAsync(unitOfWork.Product).WithMessage("The resource was not found");
        }
        ...
        protected void ValidateName(bool isUpdate = false)
        {
            ...
            if (!isUpdate)
            {
                rule.MustBeUniqueNameAsync(unitOfWork.Product).WithMessage("The name already exist");
            }
            else
            {
                rule.MustBeUniqueNameIdAsync(unitOfWork.Product).WithMessage("The name already taken");
            }
        }
    }
}